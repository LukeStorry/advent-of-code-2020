...
fn fields_are_valid(passport: &&Passport) -> bool {
    contains_fields(passport) &&
        (**passport).iter().all(|(&key, &value)|
            match key {
                "byr" => (1920..=2002)
                    .contains(&value.parse().unwrap_or(0)),
                ...
                "hgt" if value.ends_with("cm") => (150..=193).contains(
                    &value.get(..value.len() - 2).unwrap_or_default().parse().unwrap_or(0)),

                "hgt" if value.ends_with("in") => (59..=76).contains(
                    &value.get(..value.len() - 2).unwrap_or_default().parse().unwrap_or(0)),
...
mod tests {
    ...
    #[test]
    fn test_fields_are_valid__with_invalid_examples() {
    ...
    #[test]
    fn test_fields_are_valid__with_valid_examples() {
